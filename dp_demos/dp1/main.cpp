...
long long dp(int idx, bool less, bool hasForbidden, const string& num, vector<vector<vector<long long>>>& memo) {
    if (idx == num.size()) return hasForbidden;
    if (memo[idx][less][hasForbidden] != -1) return memo[idx][less][hasForbidden];
    ...
    cout << "idx: " << idx << " less: " << less << " hasForbidden: " << hasForbidden << ", " << res << endl;
    return res;
}
